...
gsap.utils.toArray('.list-rotator-wrapper').forEach((listRotatorWrapper) => {
    ...
    if (!isMobile()) {
        // Calculate based on actual list height
        const listHeight = listRotator.offsetHeight;
        const scrollDistance = listHeight - window.innerHeight + 200; // +200 for buffer
        ...
        rTimeline.from(listItems, {
            duration: 1,
            opacity: 0.1,
            y: 20,
            stagger: 0.3 // Adjust stagger based on number of items
        });
        ...
        gsap.to(listRotatorTitle, {
            scrollTrigger: {
                ...
                scrub: true,
                pinSpacing: false,
                onEnter: () => gsap.to(listRotatorTitle, {duration: 0.3, opacity: 1}),
                onLeave: () => gsap.to(listRotatorTitle, {duration: 0.3, opacity: 0}),
                onEnterBack: () => gsap.to(listRotatorTitle, {duration: 0.3, opacity: 1}),
                onLeaveBack: () => gsap.to(listRotatorTitle, {duration: 0.3, opacity: 0}),
            }
        });
    }
});
